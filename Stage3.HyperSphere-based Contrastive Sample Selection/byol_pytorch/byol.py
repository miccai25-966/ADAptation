...
def loss_fn_sphere(x, y, m=4.0):
    x = F.normalize(x, dim=-1, p=2)
    y = F.normalize(y, dim=-1, p=2)
    cosine_similarity = torch.matmul(x, y.T)  
    theta = torch.acos(cosine_similarity.clamp(-1.0, 1.0)) 
    theta_scaled = m * theta  
    total_loss = torch.mean(theta_scaled ** 2) 
...
class BYOL(nn.Module):
    ...
    def forward(
        ...
        loss_one = loss_fn_sphere(online_pred_one, target_proj_two.detach()) 
        loss_two = loss_fn_sphere(online_pred_two, target_proj_one.detach()) 
